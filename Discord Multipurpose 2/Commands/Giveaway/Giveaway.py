...
class giveaway(commands.Cog):
    ...
    async def giveaway(self,ctx,duration: str = None,channel:str = None,*,prize = None):
        global gw
        if channel == None:
        ...
        if duration == None:
            await ctx.send(':information_source: Usage: !giveaway `<duration (Example: 10S)>``<Channel>` `<Prize>`')
            return
