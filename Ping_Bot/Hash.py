...
async def on_message(message):
    ...
    if guild in rules:
        if id_ in rules[guild]:
            for x in rules[guild][id_]:
                if message.embeds == []:
                    if all(key_.replace('+','') in content for key_ in rules[guild][id_][x]['Keyword']):
                        text = ''
                        role = discord.utils.get(message.guild.roles,id = rules[guild][id_][x]['Role'])
                        if not rules[guild][id_][x]['Text'] == 'NONE':
                            text = f"{rules[guild][id_][x]['Text'].replace('$','')} {role.mention}"
                        else:
                            text = f"{role.mention}"
                    ...
                    for y in em.keys():
                        if y == 'fields':
                            ...
                            for z in content:
                                for yz in z.values():
                                    if all(key_.replace('+','') in str(yz) for key_ in rules[guild][id_][x]['Keyword']):
                            ...
                            content = str(em.get('footer')['text'])   
                            if all(key_.replace('+','') in content for key_ in rules[guild][id_][x]['Keyword']):
                                role = discord.utils.get(message.guild.roles,id = rules[guild][id_][x]['Role'])
                                if not rules[guild][id_][x]['Text'] == 'NONE':
                                    text = f"{rules[guild][id_][x]['Text'].replace('$','')} {role.mention}"
