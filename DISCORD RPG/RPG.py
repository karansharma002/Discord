...
async def fight(ctx):
    ...
    for x in Fight_Entries:
        ...
        max_rank1 = 0
        max_rank2 = 0
        for nm,y in enumerate(required):
            if nm == 0:

                if data[user] <= y and max_rank1 == 0:
                    print(data[user])
                    max_rank1 = required[nm]
                
                if data[x] <= y and max_rank2 == 0:
                    print(data[x])
                    max_rank2 = required[nm]
            
            else:
                if data[user] < y and max_rank1 == 0 and data[user] >= required[nm-1]:
                    print(data[user])
                    max_rank1 = required[nm-1]
                
                if data[x] < y and max_rank2 == 0 and data[x] >= required[nm-1]:
                    print(data[x])
                    max_rank2 = required[nm-1]
        ...
        if max_rank1 == max_rank2:
            ...
            if not author1 in Fight_Entries:
                ...
                confirmed.append(author1)
            
            if not author2 in Fight_Entries:
                Fight_Entries.append(author2)
                confirmed.append(author2)
            us = await bot.fetch_user(int(x))
            await ctx.send(f'<:miradrip:846241090721546263> ({ctx.author.mention}) and ({us.mention}) <:maskedanger:846241397777760306> will be facing off, watch the stream and wait until your fight is called in the `Announcements Channel`.')
            return
        
        elif (required.index(max_rank2) - required.index(max_rank1)) + 1 >= 0 and (required.index(max_rank2) - required.index(max_rank1)) + 1 <= 2:
            print((required.index(max_rank2) - required.index(max_rank1)) + 1)
            print('TRIGGERED 1')
            match_queue[author1] = author2
            if not author1 in Fight_Entries:
                Fight_Entries.append(author1)
    ...
    if author2 in Fight_Entries:
        ...
        try:
            ...
            if msg.content.lower() in ('yes','y'):
                ...
                confirmed.append(author1)
                confirmed.append(author2)
                await ctx.send(f'<:miradrip:846241090721546263> ({ctx.author.mention}) and ({user.mention}) <:maskedanger:846241397777760306> will be facing off, watch the stream and wait until your fight is called in the `Announcements Channel`.')
                return
...
async def rank(ctx):
    ...
    def return_pic(user):
        ...
        required = [5000,10000,20000,30000,35000,40000,50000,60000,70000,100000,125000,150000]
        for nm,x in enumerate(required):
            if current_rank <= x:
        ...
        if not str(user.id) in Fight_Entries:
            ...
            for x in match_queue:
                if x == str(user.id):
                    ...
                    await ctx.send(f'<:kujohshook:844720035943022592> {user.mention} has won the FIGHT against {author.mention}')
                    data[x] += win
                    ...
                    confirmed.remove(str(author.id))
                    confirmed.remove(str(user.id))
                    return
...
@has_permissions(administrator = True)
@bot.command()
async def setrank(ctx,user:discord.User = None,point:int  = None):
    if not user:
        await ctx.send('information_source: Command Usage: !setrank `<@user>` `<VALUE>`')
        return
    ...
    await ctx.send(':white_check_mark: Rank has been SET!')
